    ...
    linkerrortest => sub (%prm, %tml) {
        ...
        if %prm<linkerrortest>:exists and +%prm<linkerrortest>.keys {
            ...
            my %titles = <no-file no-target unknown remote >
                    Z=> 'Links to missing files', 'Links to non-existent interior targets',
                        'Unknown target schema (typo in L<> ?)', 'Urls with http/s with bad host or 404';
            for <remote no-target unknown no-file> -> $type {
                ...
                for %object.sort.map(|*.kv) -> $fn, $resp {
            ...
            $rv = "<div><h2>Link Tests</h2>Link error test (LET) plugin has not run</div>"
        }
        $rv
    },
)